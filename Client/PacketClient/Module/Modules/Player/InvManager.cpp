...
InvManager::InvManager() : IModule(0, Category::PLAYER, "Manages your inventory") {
	...
	registerBoolSetting("2B2TSwordPVP", &bbtt, bbtt);
	registerBoolSetting("Swing", &swing, swing);
	registerIntSetting("Sword", &swordSlot, swordSlot, 1, 9);
	registerIntSetting("Pot", &potSlot, potSlot, 1, 9);
	registerIntSetting("CrossBow", &CrSlot, CrSlot, 1, 9);
	registerIntSetting("Totem", &ToSlot, ToSlot, 1, 9);
	registerIntSetting("Pickaxe", &pickSlot, pickSlot, 1, 9);
	registerIntSetting("Axe", &axeSlot, axeSlot, 1, 9);
	registerIntSetting("Crystal", &blockSlot, blockSlot, 1, 9);
	...
	if (bbtt) {
		...
		for (int n = 0; n < 36; n++) {
			...
			if (stack->item != nullptr) {
				...
				if (stack->getItem()->isTrident() && currentDamage > SwordDamage) {
					SwordDamage = currentDamage;
					Sword = n;
				}
				if (stack->getItem()->isGapple() && selectedItem->count < BlockCount) {
					//PickaxeDamage = currentDamage;
					Pickaxe = n;
				}
				if (stack->getItem()->isStrength()) {
					AxeDamage = currentDamage;
					Axe = n;
				}//if (g_Data.getLocalPlayer()->getSelectedItemId() == 374 == 300)
				if (stack->getItem()->isCrystal() && selectedItem->count < BlockCount) {
					//BlockCount = blockCount;
					Block = n;
				}
				if (stack->getItem()->isCross() && selectedItem->count < BlockCount) {
					//BlockCount = blockCount;
					CrossBow = n;
				}
				if (stack->getItem()->isTotem() && selectedItem->count < BlockCount) {
					//BlockCount = blockCount;
					Totem = n;
				}
			}
		...
		if (Axe != axeSlot - 1) inv->swapSlots(Axe, axeSlot - 1);
		if (Block != blockSlot - 1) inv->swapSlots(Block, blockSlot - 1);
		//if (Pot != potSlot - 1) inv->swapSlots(Pot, potSlot - 1);
		//if (CrossBow != CrSlot - 1) inv->swapSlots(CrossBow, CrSlot - 1);
		//if (Totem != ToSlot - 1) inv->swapSlots(Totem, ToSlot - 1);
	}
