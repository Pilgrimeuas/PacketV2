...
void RenSurround::onEnable() {
	...
	if (g_Data.getLocalPlayer() != nullptr) {
		...
		centerblock = &get_center(g_Data.getLocalPlayer()->getPos()->x, g_Data.getLocalPlayer()->getPos()->y, g_Data.getLocalPlayer()->getPos()->z);
		vec3_t moveVec;
...
void RenSurround::onTick(C_GameMode* gm) {
	...
	if (gm->player != nullptr) {
		if (centerblock != 0 && center) {
			double x_diff = abs(centerblock->x - g_Data.getLocalPlayer()->getPos()->x);
			double z_diff = abs(centerblock->z - g_Data.getLocalPlayer()->getPos()->z);
		}
