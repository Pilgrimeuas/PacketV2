...
bool sound4 = false;
bool echests = false;
Surround::Surround() : IModule(0, Category::PLAYER, "surround urself with obi and such") {
	...
	if (blockLegacy->material->isReplaceable) {
		...
		if (foundCandidate) {
			...
			if (moduleMgr->getModule<Surround>()->playSound)
				if (sound1) {
					g_Data.getLocalPlayer()->pointingStruct->playSound("block.itemframe.place", *g_Data.getLocalPlayer()->getPos(), 1.f, 1.f);
				}
			if (sound2) {
				g_Data.getLocalPlayer()->pointingStruct->playSound("respawn_anchor.ambient", *g_Data.getLocalPlayer()->getPos(), 1.f, 1.f);
			}
			if (sound3) {
				g_Data.getLocalPlayer()->pointingStruct->playSound("block.end_portal_frame.fill", *g_Data.getLocalPlayer()->getPos(), 1.f, 1.f);
			}
			if (sound4) {
				g_Data.getLocalPlayer()->pointingStruct->playSound("random.levelup", *g_Data.getLocalPlayer()->getPos(), 1.f, 1.f);
...
void Surround::onLevelRender() {
	...
	if (rotations.GetSelectedEntry().GetValue() == rotations_arora) {
		...
		if (!moduleMgr->getModule<ADisabler>()->isEnabled())
			moduleMgr->getModule<ADisabler>()->setEnabled(true);
	}
	int blocks_placed = 0;
