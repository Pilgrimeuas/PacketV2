...
Killaura::Killaura() : IModule(0, Category::COMBAT, "Attacks entities") {
	...
	registerBoolSetting("SB", &cps, cps);
	...
	registerIntSetting("MaxAPS", &maxAPS, maxAPS, 1, 30);
	registerIntSetting("MinAPS", &minAPS, minAPS, 1, 30);
}
...
void Killaura::onSendPacket(C_Packet* packet) {
	...
	if (strcmp(g_Data.getRakNetInstance()->serverIp.getText(), "ntest.easecation.net") == 0) {
		if (packet->isInstanceOf<LevelSoundEventPacket>() && !targetList.empty()) {
			LevelSoundEventPacket* soundEventPacket = reinterpret_cast<LevelSoundEventPacket*>(packet);
			if (soundEventPacket->sound == 42) //sound 42是空挥手时的数值 也会被计算进CPS 但是攻击的时候不发那个包
				soundEventPacket->sound = 0;
		} //绕过EaseCation服务器CPS检测
	}
