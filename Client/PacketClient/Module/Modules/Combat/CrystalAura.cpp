...
CrystalAura::CrystalAura() : IModule(0x0, Category::COMBAT, "Crystal Aura by OldGreggo5081 & weedelbhoy1") {
...
void findEntity16(C_Entity* currentEntity16, bool isRegularEntity) {
	...
	std::string TargetUtilName = currentEntity16->getNameTag()->getText();
...
bool CrystalAura::tryCrystalAura(vec3_t CrystalAura) {
	CrystalAura = CrystalAura.floor();

	C_Block* block = g_Data.getLocalPlayer()->region->getBlock(vec3_ti(CrystalAura));
	C_BlockLegacy* blockLegacy = (block->blockLegacy);
	if (blockLegacy->material->isReplaceable) {
		vec3_ti blok(CrystalAura);
		// Find neighbour
	...
	if (!targetList16.empty()) {
		if (g_Data.getLocalPlayer()->getSelectedItemId() == crystal, nukkitcrystal) {
			...
			if (place == 0) {
				...
				if (!tryCrystalAura(blockSide1)) {
					blockSide1.y -= 0;
					blockSide1.x -= 0;
				}

				if (!tryCrystalAura(blockSide2)) {
					blockSide2.y += 0;
					blockSide2.x += 0;
				}

				if (!tryCrystalAura(blockSide3)) {
					blockSide3.y -= 0;
					blockSide3.z -= 0;
				}

				if (!tryCrystalAura(blockSide4)) {
					blockSide4.y += 0;
					blockSide4.z += 0;
				}

				//blockSide 5 to 8 are placing 1 up around the player
				if (!tryCrystalAura(blockSide5)) {
					blockSide5.x -= 0;
				}

				if (!tryCrystalAura(blockSide6)) {
					blockSide6.x += 0;
				}

				if (!tryCrystalAura(blockSide7)) {
					blockSide7.z -= 0;
				}

				if (!tryCrystalAura(blockSide8)) {
					blockSide8.z += 0;
				}

				//blockSide 9 to 12 at players below and around
				if (!tryCrystalAura(blockSide9)) {
					blockSide9.y -= 0;
					blockSide9.x -= 0;
				}

				if (!tryCrystalAura(blockSide10)) {
					blockSide10.y -= 0;
					blockSide10.x += 0;
				}

				if (!tryCrystalAura(blockSide11)) {
					blockSide11.y -= 0;
					blockSide11.z -= 0;
				}

				if (!tryCrystalAura(blockSide12)) {
					blockSide12.y -= 0;
					blockSide12.z += 0;
				}
			}
