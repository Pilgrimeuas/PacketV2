...
AutoCrystal::AutoCrystal() : IModule(0x0, Category::COMBAT, "Automatically place a crystal at the enemy's location and exploade it speedily.") {
...
void findEntityK3R(C_Entity* currentEntity, bool isRegularEntity) {
	static auto AutoCrystalMod = moduleMgr->getModule<AutoCrystal>();
...
bool checkSurrounded21(C_Entity* ent) {
	...
	for (auto blocks : blockChecks) {
		if (!checkTargCollision12(&blocks->toVector3(), ent)) {
			return false;
		}
	}
	return true;
...
std::vector<vec3_t*> getGucciPlacement21(C_Entity* ent) {
	...
	for (auto blocks : blockChecks) {
		...
		if (!checkTargCollision12(&blocks->toVector3(), ent)) {  // very efficient coding here
