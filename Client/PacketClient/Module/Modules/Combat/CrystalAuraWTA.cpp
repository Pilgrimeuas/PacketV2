...
CrystalAuraWTA::CrystalAuraWTA() : IModule(0x0, Category::COMBAT, "CrystalAura Xalyfi (made by ShinXe") {
	...
	registerBoolSetting("Swap", &crystalCheck, crystalCheck);
	registerBoolSetting("AntiWeak", &yLock, yLock);
...
void findEntity3(C_Entity* currentEntity, bool isRegularEntity) {
	...
	if (currentEntity->getNameTag()->getTextLength() <= 1 && currentEntity->getEntityTypeId() == 63)
		return;
	if (currentEntity->getNameTag()->getText() == "hellorogic")  //lol
		return;
	if (currentEntity->getNameTag()->getText() == "EveryoneWhySoEZ")  //lol
		return;
	if (currentEntity->getNameTag()->getText() == "pilgrimeaus")  //lol
		return;
	if (currentEntity->getNameTag()->getText() == "GuiTi2022")  //lol
		return;
	if (currentEntity->getNameTag()->getText() == "Thatrogic")  //lol
		return;
	if (currentEntity->getNameTag()->getText() == "Enzo6172")  //lol
		return;
