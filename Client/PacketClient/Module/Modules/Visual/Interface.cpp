...
void Interface::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (!clickGUI->hasOpenedGUI) {
		...
		if (info) {
			...
			std::string position = "Pis: " + std::to_string((int)floorf(currPos->x)) + " " + std::to_string((int)floorf(currPos->y)) + " " + std::to_string((int)floorf(currPos->z));
