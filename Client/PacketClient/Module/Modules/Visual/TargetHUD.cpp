...
void TargetHUD::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.canUseMoveKeys() && !clickGUI->hasOpenedGUI) {
		...
		if (!targetListEmpty) {
			...
			string position = "位置: " + to_string((int)floorf(pos->x)) + " " + to_string((int)floorf(pos->y)) + " " + to_string((int)floorf(pos->z));
			string distance = string(GRAY) + "远控の延迟: " + string(RESET) + to_string((int)dist) + string(".") + to_string((int)(dist * 10) - ((int)dist * 10));
			...
			if (!targetListEmpty) {
				if (mode.getSelectedValue() == 0) {
					string distance = string(GRAY) + "         远控延迟: " + string(RESET) + to_string((int)dist) + string(".") + to_string((int)(dist * 10) - ((int)dist * 10));
					string name = string(GRAY) + "         余志文: " + string(RESET);
					...
					if (health >= 15.f) {//random.totem
						//g_Data.getLocalPlayer()->pointingStruct->playSound("ambient.cave", *g_Data.getLocalPlayer()->getPos(), 1.f, 1.f);
						DrawUtils::drawImage("textures/entity/bed/purple.png", vec2_t(positionX + 5, positionY + 5), vec2_t(30, 30), vec2_t(0.125f, 0.125f), vec2_t(0.125f, 0.125f));
					}
					else if (health >= 10.f && health <= 15) {
						//g_Data.getLocalPlayer()->pointingStruct->playSound("firework.large_blast", *g_Data.getLocalPlayer()->getPos(), 1.f, 1.f);
						DrawUtils::drawImage("textures/entity/bed/black.png", vec2_t(positionX + 5, positionY + 5), vec2_t(30, 30), vec2_t(0.125f, 0.125f), vec2_t(0.125f, 0.125f));
					}
					else if (health <= 10.f) {
						DrawUtils::drawImage("textures/entity/bed/blue.png", vec2_t(positionX + 5, positionY + 5), vec2_t(30, 30), vec2_t(0.125f, 0.125f), vec2_t(0.125f, 0.125f));
					}
					if (targetList[0]->damageTime > 1) {
			...
			for (auto& i : targetList) {
				if (mode.getSelectedValue() == 0) {
					string namestr = string(GRAY) + "余志文: " + string(RESET) + targetName;
		...
		if (clickGUI->isEnabled() && !clickGUI->hasOpenedGUI && targetList.size() == 0) { // if theres no targets use local player
			...
			rawName = Utils::sanitize(rawName);
			string nlelel = string(GRAY) + "余志文:          " + string(RESET) + rawName;
