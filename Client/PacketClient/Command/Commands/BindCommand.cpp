...
BindCommand::BindCommand() : IMCCommand("bind", "绑定功能到某个按键", "<module> <key>") {
	registerAlias("b");
}
...
bool BindCommand::execute(std::vector<std::string>* args) {
	...
	if (!modOpt.has_value()) {
		std::string textStr = "找不到功能: " + moduleName;
	...
	if (args->size() >= 3) {
		...
		if (key.size() > 1) {
			...
			if (key == "none") {
				mod->setKeybind(0x0);
				std::string textStr = "成功取消绑定 " + std::string(mod->getRawModuleName());
				//auto notification = g_Data.addNotification("Commands:", textStr);
				return true;
			}
