...
bool ConfigCommand::execute(std::vector<std::string>* args) {
	if (args->at(1) == "load" || args->at(1) == "l") {
		...
		clientMessageF("[Packet] %s成功载入配置 %s%s%s!", GREEN, GRAY, configMgr->currentConfig.c_str(), GREEN);
		clientMessageF("[Packet] %s成功保存 %s%s%s!", GREEN, GRAY, SettingMgr->currentSettings.c_str(), GREEN);
		return true;
	}
	return false;
}