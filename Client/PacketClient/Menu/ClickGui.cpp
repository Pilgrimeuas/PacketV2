...
static constexpr float textSize = 0.85;
...
static constexpr float crossSize = textHeight / 0.5f;
...
void ClickGui::renderTooltip(string* text) {
	...
	DrawUtils::drawText(textPos, text, MC_Color(0, 0, 255), 1);
}
	...
	if (ourWindow->isExtended) {
		...
		for (auto& mod : moduleList) {
			...
			vec4_t rectPos = vec4_t(
				...
				currentYOffset + textHeight + (textPadding * 4));
			...
			if (allowRender) {
				if (!ourWindow->isInAnimation && !isDragging && rectPos.contains(&mousePos)) {
					...
					DrawUtils::fillRectangleA(rectPos, mod->isEnabled() ? MC_Color(25, 25, 25, clickGUI->opacity) : MC_Color(25, 25, 25, clickGUI->opacity));
				}
			}

			// Text
			if (allowRender) DrawUtils::drawCenteredString(textPos, &textStr, textSize, mod->isEnabled() ? MC_Color(0, 0, 255) : MC_Color(255, 255, 255), true);
					...
					if (clickMod->isExtended) {
						...
						for (auto setting : *settings) {
							...
							case ValueType::BOOL_T: {
								...
								DrawUtils::fillRectangleA(rectPos, MC_Color(0, 255, 0, clickGUI->opacity));
								...
								// Checkbox
								{
									...
									DrawUtils::drawRectangle(boxPos, MC_Color(0, 255, 255), isFocused ? 1 : 0.8f, 0.5f);

									if (setting->value->_bool) {
										DrawUtils::setColor(244, 0, 244, 1);
